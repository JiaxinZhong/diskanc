...
draw(shift(leg_shift)*(loc_c -- loc_c+1.5*w*Z), L=Label('Sound center',p=label_fontsize,position=EndPoint), align=Z-0.1Y);
/* draw(shift(leg_shift)*(loc_c -- loc_c+2*w*Y+w*Z), L=Label('Sound center',p=label_fontsize,position=EndPoint)); */

// Draw the error points
real[] theta = {0,45,45,90};
real[] phi = {90,45,135,90};
real PI = 3.14159;
triple[] loc_e = {};
...
/* path box2 = box((0,0), (disk_r, -disk_thick)); */
/* path3 box3 = path3(box2, plane = YZplane); */
real plate_thick = 0.018;
surface plate1p2 = zscale3(zmax) * xscale3(2*xmax) * yscale3(plate_thick) * unitcube;
draw(shift(-plate_thick*Y-xmax*X)*plate1p2, cyan + opacity(0.2), light=nolight);
/* path3[] plate1p2_frame = box(xmax*X,-xmax*X-plate_thick*Y+zmax*Z); */
/* draw(plate1p2_frame); */
/* draw(shift(zmax*Z+xmax*X)*(O--0.1X+0.1Z),L=Label('Infinitely large surface',p=label_fontsize)); */
